...
@section('header-new')
    <div class="bg-lime-500 p-4 text-center rounded-lg shadow-lg">
        <h1 class="text-2xl font-semibold text-white uppercase">Videos</h1>
    </div>
@endsection
@section('css-new')
...
@section('container-new')
    ...
    focus:ring-offset-2 transition ease-in-out duration-150 ml-4">{{ __('Agregar nuevo video') }}</button>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form class="p-4 md:p-5" method="POST" id="formAdd" action="{{ route('admin.videos.store') }}"
                            >
                            @csrf
                            <div class="grid gap-4 mb-4 grid-cols-2">
                                <div class="col-span-2">
                                    ...
                                    <input type="text" name="titulo" id="titulo"
                                        ...
                                        placeholder="Titulo del video" required="">
                                </div>
                                <div class="col-span-2">
                                    <label for="name"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                        <i class="fas fa-layer-group mr-1 my-text-color"></i> Video ID</label>
                                    <input type="text" name="video_id" id="video_id"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="Id del video" required="">
                                </div>
                                <div class="col-span-2">
                                    <label for="name"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                        <i class="fas fa-layer-group mr-1 my-text-color"></i> URL</label>
                                    <input type="text" name="url" id="url"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="Url del video" required="">
                                </div>
                            </div>
                            ...
                            <div class="grid gap-4 mb-4 grid-cols-2">
                                ...
                                <div class="col-span-2">
                                    <label for="name"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                        <i class="fas fa-layer-group mr-1 my-text-color"></i> Video ID</label>
                                    <input type="text" name="video_id" id="video_idEdit"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="Id del video" required="">
                                </div>
                                <div class="col-span-2">
                                    <label for="name"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                        <i class="fas fa-layer-group mr-1 my-text-color"></i> URL</label>
                                    <input type="text" name="url" id="urlEdit"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="Url del video" required="">
                                </div>
                    ...
                    <table id="example" class="stripe hover "
                        ...
                        <thead class="text-xs text-white uppercase bg-blue-500 dark:text-white">
                            <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                                ...
                                <th class="py-3 px-6 text-center">VideoID</th>
                                        ...
                                        <div class="flex item-center justify-center">
                                            <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                                @if ($item->estado === 'A')
                                                    <button class="inline-flex items-center desactivar"
                                                        data-video-id="{{ $item->id }}" id="deactivateVideoButton"
                                                ...
                                                @else
                                                    <button class="inline-flex items-center activar"
                                                        data-video-id="{{ $item->id }}" id="activateVideoButton"
                                                        title="Activar video">
                                            ...
                                            <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                                ...
                                                <button class="inline-flex items-center editar-marca"
                                                    id="readProductButton" data-video-id="{{ $item->id }}"
                                                    data-video-titulo="{{ $item->titulo }}"
                                                    data-video-video_id="{{ $item->video_id }}" 
                                                    data-video-url="{{ $item->url }}" data-toggle="modal"
                                            ...
                                            <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                                ...
                                                <form class="form-delete"
                                                    ...
                                                    @csrf
                                                    {{ method_field('DELETE') }}
                                                    <input type="hidden" name="video_name" value="{{ $item->titulo }}">
        ...
        <script>
            $(document).ready(function() {
                ...
                $('#example tbody').on('click', '.editar-marca', function() {
                    ...
                    const videoTitulo = $(this).data('video-titulo');
                    const videoUrl = $(this).data('video-url');
                    const videoVideo_id = $(this).data('video-video_id');

                    // Rellenar el formulario en el modal con los datos obtenidos
                    $('#tituloEdit').val(videoTitulo);
                    $('#video_idEdit').val(videoVideo_id);
                    $('#urlEdit').val(videoUrl);
                    $('#idEdit').val(videoId);
                    // Establecer la acción del formulario para editar la categoría
                    const editForm = $('#editForm');
                    const actionUrl =
                        "{{ route('admin.videos.update', ['video' => ':video']) }}";
                    const updatedActionUrl = actionUrl.replace(':video', videoId);
                ...
                $('.form-delete').submit(function(e) {
                    e.preventDefault();
                    const videoName = e.target.querySelector("input[name='video_name']").value;
                    Swal.fire({
                        title: `¿Estás seguro de que deseas eliminar el video "${videoName}"?`,
                ...
                $('#example tbody').on('click', '.desactivar', function() {
                    ...
                    $.ajax({
                        url: 'admin/videos/' + videoId + '/status',
                        type: 'PUT',
                        data: {
                            _token: '{{ csrf_token() }}',
                            _method: 'PUT',
                        ...
                        success: function(response) {
                            ...
                            console.log('Desactivar el video:', response);
                            location.reload();
                        },
                        error: function(xhr) {
                            // Manejar errores si es necesario
                            console.error('Error al desactivar el video:', xhr);
                        }
                    });
                });
                ...
                $('#example tbody').on('click', '.activar', function() {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            console.log('Activar el video:', response);
                            location.reload();

                        },
                        error: function(xhr) {
                            // Manejar errores si es necesario
                            console.error('Error al activar el video:', xhr);
                        }
                    });
                });
            });
        ...
        <script>
            function validarFormulario() {
                ...
                if (titulo === '' || url === '' || video_id === '') {
                    ...
                    icon: 'success',
                    title: '¡Éxito!',
                    text: 'El video fue creado con éxito.',
                }).then(() => {
            ...
            function validarFormularioEdit() {
                ...
                const url = document.getElementById('urlEdit').value.trim();
                const video_id = document.getElementById('video_idEdit').value.trim();
